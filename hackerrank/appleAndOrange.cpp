...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    int64_t apple_to_house = s - a;
    int64_t orange_to_house = t - b;
    int64_t counter_apples(0), counter_oranges(0);
    for_each(apples.begin(), apples.end(), [&](int i) {
        if (i >= apple_to_house && i <= t - a)
            ++counter_apples;
    });
    cout << counter_apples << '\n';
    for_each(oranges.begin(), oranges.end(), [&](int j) {
        if (j <= orange_to_house && j >= s - b)
            ++counter_oranges;
    });
    cout << counter_oranges << '\n';        
}
